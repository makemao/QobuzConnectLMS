...
**Volume changed on LMS → app**: the state loop compares each status `mixer volume` with the last volume known to the app (`_known_volume`); a difference fires the backend `volume_change` callback, and `QobuzPlayer._on_backend_volume_change` sends `RNDR_SRVR_VOLUME_CHANGED`. Idle, only an LMS `mixer` event triggers a read. Muted is reported as 0. An app volume change is not echoed, and differences in the next `APP_VOLUME_GRACE_SECONDS` are ignored (stale reads).
